...
bool check_full_line(char m[21][10], int rowIndex){ // check per line (rowIndex), !! Check can only be done after check_collision, else live piece may be counted
  int sum=0;
  for (int c = 0 ; c < 10; c++){
    if(m[rowIndex][c]!='-'){
      sum = sum + 1;
  ...
  return (sum == 10); // sum == 10 means whole row has an eleme
}

void shift_down(char m[21][10],int rowIndex){ //shift all (rowIndex) tiles down after clearing line by numOfRows
  
  for(int c = 0; c<10; c++){
    for(int r = rowIndex; r<rowIndex; r++){
      if m[r][c] !=
    }
  }
}

void clear_line(char m[21][10], int rowIndex, int numOfRows){ //clear the line, keep in mind top pieces are yet to be shifted down
  for (int c = 0; c < 10; c++){
    int
    m[rowIndex][c] = '-';
  }
...
bool check_collision(char m[21][10], char color){
  for (int i = 20; i>=1; i--){ //iteration from bottom-up to make check more efficient, i>=1 to ignore padding
    for (int j = 0; j<10; j++){
      if (m[i][j] == color){
